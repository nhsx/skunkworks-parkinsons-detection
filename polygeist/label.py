...
def generate_densities_and_map_coordinates(
    raw_image, stride, raw_threshold, class_threshold
):
    """
...
def autogain_background(
    image, xin=(200, -100), yin=(100, 200), threshold=0.95, channel=0, value=255
):
    """
    ...
    mask_coords = np.asarray(
        np.column_stack(np.ma.where(image[:, :, channel] > background_mean * threshold))
    )
...
def process_file(
    ...
    densities, coordinates, map_coordinates = generate_densities_and_map_coordinates(
        raw_image=raw,
        stride=stride,
...
def create_colourised_density_map(
    raw, raw_threshold, densities, stride, class_threshold, identify_rois=False
):
    """
    ...
    spatial_density_map = colourmap_veridis(
        spatial_density_map.reshape(raw.shape[0] * raw.shape[1])
    )
    density_map_array = (
        spatial_density_map.reshape(raw.shape[0], raw.shape[1], 3) * 255
    ).astype(int)
        ...
        for xi, x in enumerate(np.arange(0, xx, stride)):
            for yi, y in enumerate(np.arange(0, yy, stride)):
                if densities[yi, xi] < class_threshold:
                    reduced_brightness_density_map[
                        y : y + stride, x : x + stride, :
                    ] *= (2 * class_threshold)
                else:
                    reduced_brightness_density_map[
                        y : y + stride, x : x + stride, :
...
def process_files_and_folders(
    ...
    if os.path.isdir(input_file_or_folder):
        ...
        for root, directory_names, filenames in os.walk(input_file_or_folder):
            for filename in (
                fnmatch.filter(filenames, "*.svs")
                if not synthetic
                else fnmatch.filter(filenames, "*.jpg")
            ):
    ...
    for file_number, file in enumerate(list_of_files):
        ...
        image, raw, densities, coordinates, map_coordinates = process_file(
            file,
            stride=stride,
            raw_threshold=raw_threshold,
            class_threshold=class_threshold,
            apply_automatic_background_removal=auto_remove_background,
        )
        ...
        if not skip_jpeg:
            ...
            reduced_brightness_density_map, density_map = create_colourised_density_map(
                raw=raw,
                raw_threshold=raw_threshold,
                densities=densities,
