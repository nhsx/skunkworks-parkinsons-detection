...
class Microscope:
    ...
    leica_r = pc.Illuminant.from_file_with_range(
        "spectral/scanner/Leica_calibrated_r.csv", wl
    )
    leica_g = pc.Illuminant.from_file_with_range(
        "spectral/scanner/Leica_calibrated_g.csv", wl
    )
    leica_b = pc.Illuminant.from_file_with_range(
        "spectral/scanner/Leica_calibrated_b.csv", wl, interp="linear"
    )

    # Raw functions of the sensor reported in the same presentation
    raw_leica_r = pc.Illuminant.from_file_with_range(
        "spectral/raw_sensors//Leica_r.csv", wl
    )
    raw_leica_g = pc.Illuminant.from_file_with_range(
        "spectral/raw_sensors//Leica_g.csv", wl
    )
    raw_leica_b = pc.Illuminant.from_file_with_range(
        "spectral/raw_sensors//Leica_b.csv", wl
    )

    # Lightsource (INGAN)
    ls = pc.Illuminant.from_file_with_range("spectral/raw_sensors/LXA7-PW57.csv", wl)

    def response(self, filter_function, power=1.0, plot=False):
