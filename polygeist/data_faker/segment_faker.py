...
def generate_fake_segment(x_size=100, y_size=100, dab_contrib=1.):
    # Leica responses
    responses = np.array([[7.10357511, 12.61506218, 13.59695489],
...
def write_fake_data_segments(directory, pathology_present=False, case_name='PDXXX', slide_index=17
                             , segments=10, pathology='A-syn', stride=512):
    for i in np.arange(0, segments):
        im = generate_fake_segment(x_size=stride, y_size=stride, dab_contrib=1. if pathology_present else 0.25)
